...
export class GroupService {
  ...
  async create(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const client of clients) {
        ...
        await manager.getRepository(ClientEntity).save(checkClientLog);
  ...
  async getGroups(
    ...
    const [groupsData, itemCount] = await queryBuilder.getManyAndCount();
  ...
  async findOne(id: string, user: UserEntity): Promise<GroupEntity> {
    ...
    const groupWithLeaderId = {
      ...
      officeName: group.staff?.office?.name || group.officeName,
    };
